...
def timed(f):
    ...
    def wrapped(*args):
        ...
        print("Time of execution is: f est {t:.0f} s".format(t=(time.time()) - t))
    return wrapped
...
class Processer:
    ...
    def process(self, f, g, header=True, debug=False):
        if header:
            head = next(f)
            g.write(head)
        if debug:
...
def clean_element(el, d):
    el = el.strip()
    if el in d:
        if type(d[el]) == type(int):
            el = ",".join([(el + "_" + str(i) for i in range(d[el]))])
        if type(d[el]) == type(str):
            el = d[el]
    return el
...
DEFAULT_PROCESSER = Processer(DEFAULT_MAPPER, get_mapper(DEFAULT_PRE_MAPPING_FUNCTIONS),
                              lambda t: str(t[1]), transform_header({'': 'index'}))
